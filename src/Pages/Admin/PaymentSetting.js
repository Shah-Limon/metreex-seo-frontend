...
const PaymentSetting = () => {
  ...
  useEffect(() => {
    fetch(`https://metreex-seo-9a225b1c6d1e.herokuapp.com/payment-accounts`)
      .then((res) => res.json())
      .then((info) => setPayments(info));
  }, []);

  useEffect(() => {
    fetch(`https://metreex-seo-9a225b1c6d1e.herokuapp.com/cash-on-delivery`)
      .then((res) => res.json())
  ...
  const handleCashOnDeliveryEnable = (event) => {
    ...
    const url = `https://metreex-seo-9a225b1c6d1e.herokuapp.com/cash-on-delivery`;
    fetch(url, {
      method: "POST",
      headers: {
  ...
  const handleAddNewPaymentAccount = (event) => {
    ...
    const url = `https://metreex-seo-9a225b1c6d1e.herokuapp.com/add-payment-account`;
    fetch(url, {
      method: "POST",
      headers: {
        "content-type": "application/json",
